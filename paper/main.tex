...
\midrule
\texttt{has\_most\_favored\_nation}  & 94\% (0.36) & 96\% (0.90) & 94\% (0.93) \\
\texttt{has\_liquidated\_damages}    & 86\% (0.51) & 92\% (0.73) & 90\% (0.83) \\
\texttt{has\_exclusivity}            & 67\% (0.23) & 86\% (0.76) & 92\% (0.79) \\
\texttt{has\_anti\_assignment}       & 27\% (0.19) & \regress{22\% (0.99)} & 35\% (0.93) \\
\texttt{governing\_law} (12-way)     & 18\% (0.20) & 24\% (0.57) & 27\% (0.69) \\
\texttt{renewal\_term} (6-way)       & \regress{2\% (0.45)} & 53\% (0.64) & 55\% (0.70) \\
\texttt{expiration\_type} (3-way)    & 49\% (0.25) & 67\% (0.68) & 63\% (0.76) \\
\bottomrule
\end{tabular}
...
\textbf{Extending to CUAD reveals field-specific variance.} Applying PCL fine-tuning to the 11-field CUAD schema (Section~\ref{sec:cuad}) across the same three scales produces a mixed picture. Aggregate accuracy rises modestly (+7/+2/+10 pp at 0.5B/7B/32B, vs. +32/+19/+20 on Flights), and the per-field breakdown explains why: PCL-FT's benefit is concentrated on fields whose pretrained prior conflicts with the task distribution (\texttt{has\_anti\_assignment} 27→86\% at 32B; \texttt{governing\_law} 18→53\% at 32B). On fields whose pretrained behaviour was already correct (\texttt{has\_most\_favored\_nation}, \texttt{has\_liquidated\_damages}), PCL-FT is neutral. On some fields PCL-FT \emph{regresses} (\texttt{expiration\_type} falls from 49/67/63\% baseline to 45/47/49\% after PCL-FT; \texttt{has\_non\_compete} drops at 0.5B and 7B). The regressions appear concentrated where the training signal available to the model is weak relative to the field's semantic requirements -- plausibly because we truncate contracts to the first $\sim$2000 tokens for training efficiency and the diagnostic clauses often sit deeper in the document. The finding is important for the deployment posture: PCL-FT is not a uniform win, and per-field evaluation remains necessary even after intervention.
...
\midrule
Flights & 0.5B & 58\% & 77\% @66\% & 90\% & \textbf{96\%} @86\% \\
Flights & 7B   & 71\% & 72\% @96\% & 90\% & \textbf{93\%} @94\% \\
Flights & 32B  & 72\% & 84\% @82\% & 92\% & \textbf{94\%} @97\% \\
\midrule
CUAD    & 0.5B & 55\% & 64\% @53\% & 62\% & \textbf{66\%} @86\% \\
CUAD    & 7B   & 65\% & 68\% @90\% & 67\% & \textbf{75\%} @83\% \\
CUAD    & 32B  & 66\% & 68\% @91\% & 76\% & \textbf{81\%} @91\% \\
\bottomrule
\end{tabular}
\caption{Aggregate accuracy across all constrained fields, per dataset $\times$ scale. Cells are argmax accuracy for argmax columns and ``committed accuracy @ coverage'' for the gated columns. The PCL-FT + inference-time gating combination is the best cell in every row.}
...
\begin{itemize}
\item \textbf{PCL-FT always helps.} Every row shows PCL-FT argmax $\ge$ baseline argmax. On Flights the gap is large (+32/+19/+20 pp); on CUAD it is smaller (+7/+2/+10 pp) because CUAD's boolean majority is preserved under FT regardless of the 3-way enum training signal.
\item \textbf{Margin gating always adds a few percentage points on top of PCL-FT.} Even at 32B where PCL-FT reaches 92\%/76\%, gating adds 2--5 pp of committed accuracy at the cost of a small amount of coverage. The two interventions are not redundant.
\item \textbf{The lift from margin gating is largest at 0.5B.} On the baseline Flights 0.5B row, gating alone lifts committed accuracy from 58\% to 77\% at 66\% coverage -- equivalent to one-third of the 32-pp gap that PCL-FT itself closes. For deployments where fine-tuning is impractical, inference-time gating on a small base model already captures a meaningful portion of the PCL-FT benefit.
\end{itemize}

The combined intervention (PCL-FT + margin gating) reaches 81--96\% across our scales and datasets, dominating every other configuration. The $\theta = 0.30$ operating point is conservative; tightening it further trades coverage for precision.
